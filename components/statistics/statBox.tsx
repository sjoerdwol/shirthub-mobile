...
export default function StatBox({ currencyVisible, icon, subtitle, title, value }: StatBoxProps) {
  return (
    <View className='border border-dark-border flex-1 h-30 p-6 rounded-2xl'>
      <Text className="font-LexendSemiBold text-white/50 text-lg" testID='stat_box_title'>{title}</Text>
      <Text className="font-LexendBold mb-3 text-white/80 text-2xl" testID='stat_box_value'>
        {currencyVisible
      ...
      </Text>
      {
        ...
        <View className='flex-row items-center gap-2'>
          {icon}
          <Text className="font-Lexend text-white/50" testID='stat_box_subtitle'>{subtitle}</Text>
        </View>
      }
    </View>
  );
